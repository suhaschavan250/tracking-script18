(function () {
  ...
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script18'));
  ...
  function handleClick(event) {
    ...
    if (CONFIG.ctaMethod === 'CTA Text') {
      ...
      console.log(`[Tracking] CTA Text Match: ${matchedCTA}`);
      if (matchedCTA) {
        console.log('[Tracking] ✅ CTA matched by Text:', clickedText);
        sendToAllPlatforms('any_cta', clickedText);
      } else {
        console.log('[Tracking] ❌ CTA text did not match any configured values');
      }
    }

    // CTA Button ID Matching
    if (CONFIG.ctaMethod === 'CTA Button Id') {
      ...
      if (matchedId) {
        const isMatch = matchedId === CONFIG.ctaValue;
        console.log(`[Tracking] Found ID: "${matchedId}", Expected: "${CONFIG.ctaValue}" → Match: ${isMatch}`);

        if (isMatch) {
          console.log('[Tracking] ✅ CTA match by Button ID, firing any_cta');
          sendToAllPlatforms('any_cta', clickedText);
        } else {
          console.log('[Tracking] ❌ Button ID does not match, not firing any_cta');
        }
      } else {
        console.log('[Tracking] ❌ No ID found on clicked element or its parents');
      }
    }
  }
